"""Windows agent integration test — File Explorer (GW-042).
...
This test exercises navigation within a Win32 shell application that
uses a tree-based navigation pane and an item grid. File Explorer has
distinct accessibility characteristics compared to modern XAML apps
(Settings), including a command bar with small-icon toolbar buttons,
...
2. ``desktop.snapshot`` — capture the File Explorer accessibility tree
3. ``desktop.find`` — locate the address bar element
4. ``desktop.get_text`` — read the current path from the address bar
...
# -- Replay script: simulated Claude interaction with File Explorer -------
# This replay script models the full agent loop that Claude would execute
# when asked to "open File Explorer and read the current path". Each frame
# is one LLM response containing tool_use blocks.

FILE_EXPLORER_AGENT_REPLAY = [
    # Turn 1: List windows to find File Explorer
    {
    ...
    # Turn 2: Snapshot File Explorer to understand its structure
    {
        "stop_reason": "tool_use",
        "content_blocks": [
            {
                "type": "text",
                "text": "I can see File Explorer is running. Let me take a snapshot.",
            },
            {
                "type": "tool_use",
                "id": "tu_002",
    ...
    # Turn 3: Find the address bar element
    {
        "stop_reason": "tool_use",
        "content_blocks": [
            {
                "type": "text",
                "text": "I can see the File Explorer window with a toolbar, "
                "address bar, navigation pane, and item grid. "
                "Let me find the address bar to read the current path.",
            },
            {
                "type": "tool_use",
                "id": "tu_003",
                "name": "desktop.find",
                "input": {"window_ref": "w1", "name": "Address bar"},
            },
        ],
    },
    # Turn 4: Read the current path from the address bar
    {
        "stop_reason": "tool_use",
        "content_blocks": [
            {
                "type": "text",
                "text": "Found the address bar. Let me read its value to see the current path.",
            },
            {
                "type": "tool_use",
                "id": "tu_004",
    ...
    # Turn 5: Final summary
    {
        ...
        "content_blocks": [
            {
                "type": "text",
                "text": "Successfully opened File Explorer and read the current "
...
class TestWindowsAgentFileExplorer:
    """GW-042: Prompt-driven agent integration test for File Explorer.
    ...
    with the File Explorer application.
    """
    ...
    async def test_agent_reads_file_explorer_path(self) -> None:
        ...
        async with GuidewireServerProcess(backend="auto") as server:
            agent = AgentClient(
                server, replay_script=FILE_EXPLORER_AGENT_REPLAY, max_turns=5
            )
            result = await agent.send_prompt(
                "Open File Explorer and read the current path."
            )
    ...
    async def test_find_targets_address_bar(self) -> None:
        ...
        assert find_calls[0].input["name"] == "Address bar"
