"""Capture accessibility tree snapshots from Linux applications.

This script launches target applications (gedit, GNOME Calculator),
captures their UI accessibility trees via LinuxBackend.snapshot(), and
saves the normalized output as JSON golden fixtures.

Usage:
    python -m tests.fixtures.capture_linux_snapshots
...
FIXTURES_DIR = Path(__file__).parent / "linux"
...
def _capture_snapshot(
    ...
    """Capture the accessibility snapshot using LinuxBackend.
...
def main() -> None:
    ...
    print("Capturing Linux accessibility snapshots...")
